...
class FinancialAnalyzer:
    ...
    def generate_report(self) -> Dict:
        ...
        report = {
            ...
            "volume": {
                ...
                "ratio_to_ma20": (
                    round(float(latest["Volume"] / latest["Volume_MA20"]), 2)
                    if latest["Volume_MA20"] > 0
                    else 0
                ),
            },
        }
        ...
        if ML_AVAILABLE and len(features) >= 50:
            ...
            report["ml_prediction"] = {
                ...
                "top_features": dict(
                    sorted(
                        clf.get("feature_importance", {}).items(), key=lambda x: x[1], reverse=True
                    )[:5]
                ),
            }
    ...
    def _generate_advice(self, report: Dict) -> Dict:
        ...
        advice["signals"].append(
            {
                "indicator": "MACD",
                "status": "金叉" if macd > macd_sig else "死叉",
                "suggestion": "看涨" if macd > macd_sig else "看跌",
            }
        )

        if bb_pos > 0.8:
            advice["signals"].append(
                {"indicator": "布林带", "status": "上轨", "suggestion": "警惕回调"}
            )
        elif bb_pos < 0.2:
            advice["signals"].append(
                {"indicator": "布林带", "status": "下轨", "suggestion": "可能反弹"}
            )
        ...
        buy_cnt = sum(
            1 for s in advice["signals"] if "看涨" in s["suggestion"] or "关注" in s["suggestion"]
        )
        sell_cnt = sum(
            1 for s in advice["signals"] if "看跌" in s["suggestion"] or "谨慎" in s["suggestion"]
        )
        if buy_cnt > sell_cnt:
            advice["operation"] = "买入"
