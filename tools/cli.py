...
MCP_TOOLS = [
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {"drama_dir": {"type": "string", "description": "剧本目录路径"}},
            "required": ["drama_dir"],
        },
    },
    {
...
def mcp_call_tool(name: str, arguments: dict):
    ...
    if name == "baibai_format_check":
        ...
        return {
            "content": [
                {"type": "text", "text": f"格式校验完成: {len(result.get('results', []))} 部剧本"}
            ]
        }
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"统计完成，共 {result.get('stats', {}).get('total_dramas', 0)} 部",
                }
            ]
        }
